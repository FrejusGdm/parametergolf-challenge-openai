...
"""
Experiment 06 — 10 Layers (vs baseline 9).

Adds one extra transformer layer (NUM_LAYERS=10) and compares against the
9-layer baseline. More depth should improve modeling capacity, but increases
artifact size. Both runs use 2000 steps for fair comparison.

Workflow:
1) Clone parameter-golf repo, download 20 train shards
2) Train baseline (9 layers, default) for 2000 steps
3) Train with NUM_LAYERS=10 for 2000 steps
...
def main() -> None:
    ...
    workspace = Path("/tmp/exp06_layers")
    if workspace.exists():
    ...
    try:
        ...
        # Run 1: Baseline (9 layers, default)
        print("\n" + "=" * 60)
        print("RUNNING BASELINE (9 layers)")
        print("=" * 60)
        baseline_log = train_run(
            repo_dir,
            workspace / "baseline.log",
            f"exp06_baseline_{int(time.time())}",
        )
        baseline_metrics = parse_metrics(baseline_log)

        # Run 2: NUM_LAYERS=10
        print("\n" + "=" * 60)
        print("RUNNING NUM_LAYERS=10")
        print("=" * 60)
        layers10_log = train_run(
            repo_dir,
            workspace / "layers10.log",
            f"exp06_10layers_{int(time.time())}",
            extra_env={"NUM_LAYERS": "10"},
        )
        layers10_metrics = parse_metrics(layers10_log)
    ...
    result_payload = {
        "status": status,
        "error": error_message,
        "experiment": "06-more-layers",
        "description": "10 layers vs baseline 9 layers — comparing val_bpb and artifact size",
        ...
        "baseline_9layers": baseline_metrics,
        "variant_10layers": layers10_metrics,
    }

    # Compute deltas if both succeeded
    if baseline_metrics and layers10_metrics:
        b_bpb = baseline_metrics.get("post_quant_val_bpb")
        l_bpb = layers10_metrics.get("post_quant_val_bpb")
        if b_bpb and l_bpb:
            result_payload["delta_bpb"] = round(l_bpb - b_bpb, 6)

        b_art = baseline_metrics.get("artifact_bytes")
        l_art = layers10_metrics.get("artifact_bytes")
        if b_art and l_art:
            result_payload["delta_artifact_bytes"] = l_art - b_art

    stamp = time.strftime("%Y%m%d_%H%M%S", time.gmtime())
    run_id = f"exp06_layers_{stamp}"
    out_prefix = f"jobs/exp06/{run_id}"
    ...
    for logname in ["baseline.log", "layers10.log"]:
    ...
    # Print summary
    print("\n" + "=" * 60)
    print("EXPERIMENT 06 — 10 LAYERS RESULTS")
    print("=" * 60)
    if baseline_metrics:
        print(
            f"  Baseline (9L) val_bpb:  {baseline_metrics.get('post_quant_val_bpb', 'N/A')}"
        )
        print(
            f"  Baseline (9L) artifact: {baseline_metrics.get('artifact_bytes', 'N/A')} bytes"
        )
    if layers10_metrics:
        print(
            f"  10-layer val_bpb:       {layers10_metrics.get('post_quant_val_bpb', 'N/A')}"
        )
        print(
            f"  10-layer artifact:      {layers10_metrics.get('artifact_bytes', 'N/A')} bytes"
        )
