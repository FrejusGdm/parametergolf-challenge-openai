...
Attribution:
- Derived from OpenAI Parameter Golf baseline `train_gpt_mlx.py`:
...
class Hyperparameters:
    ...
    chunkgate_enable: bool = bool(int(os.environ.get("CHUNKGATE_ENABLE", "0")))
    chunkgate_stride: int = int(os.environ.get("CHUNKGATE_STRIDE", 4))
    chunkgate_inner_layers: int = int(os.environ.get("CHUNKGATE_INNER_LAYERS", 2))
    chunkgate_gate_temp: float = float(os.environ.get("CHUNKGATE_GATE_TEMP", 1.0))
    chunkgate_fusion_init: float = float(os.environ.get("CHUNKGATE_FUSION_INIT", 0.10))
...
class ChunkGateLite(nn.Module):
    ...
    def __init__(
        ...
        self.gate_proj = CastedLinear(dim, 1)
        self.chunk_gain = mx.ones((dim,), dtype=mx.float32)
        self.fusion = mx.ones((dim,), dtype=mx.float32) * fusion_init
        self.blocks = [
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        gate_logits = self.gate_proj(rms_norm(x)).squeeze(-1)
        gate = mx.sigmoid(gate_logits / self.gate_temp).astype(x.dtype)[..., None]

        x_comp = x[:, :: self.stride, :]
        gate_comp = gate[:, :: self.stride, :]
        x_comp = x_comp * (1.0 + self.chunk_gain.astype(x.dtype)[None, None, :] * gate_comp)
        x0_comp = x_comp
        for block in self.blocks:
            x_comp = block(x_comp, x0_comp)

        x_up = mx.repeat(x_comp, self.stride, axis=1)[:, :seqlen, :]
        return x + self.fusion.astype(x.dtype)[None, None, :] * gate * x_up
...
class SplitOptimizers:
    ...
    def __init__(self, model: GPT, args: Hyperparameters):
        ...
        matrix_key_set = set(self.matrix_keys)
        self.scalar_keys = [
            k
            for k in params
            if k != self.embed_key and k not in matrix_key_set
        ]
